(()=>{
  ...
  document.querySelector('#booking-form')?.addEventListener('submit',async event=>{event.preventDefault();if(!state.service||!state.date||!state.time)return message('Selecciona servicio, fecha y horario.',true);const submit=event.currentTarget.querySelector('[type="submit"]');submit.disabled=true;const fields=Object.fromEntries(new FormData(event.currentTarget));fields.turnstileToken=window.turnstile?.getResponse(event.currentTarget.querySelector('.cf-turnstile'))||'';message('Guardando tu solicitud…');try{const response=await fetch('/api/bookings',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({...fields,serviceId:state.service.id,date:state.date,time:state.time})});const data=await response.json();if(!response.ok)throw new Error(data.error||'No fue posible guardar la solicitud');document.querySelector('#booking-summary').innerHTML=`<strong>${state.service.name}</strong><br>${state.date} · ${state.time}<br>${state.service.location} · $${state.service.price} MXN`;document.querySelector('#booking-success').textContent=`Folio ${data.folio}. Recibimos tu solicitud.${data.notificationSent?' Enviamos una copia a tu correo.':' Conserva este folio; Makanuy confirmará contigo el horario.'}`;show('complete');window.dataLayer?.push({event:'booking_request',service_id:state.service.id,booking_folio:data.folio})}catch(error){message(error.message,true);window.turnstile?.reset(event.currentTarget.querySelector('.cf-turnstile'));submit.disabled=false}});
