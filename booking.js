(()=>{
  ...
  const services=[
    ...
    {id:'lactancia',slug:'asesoría-de-lactancia',name:'Orientación nutricional en Lactancia',minutes:60,price:350,location:'Presencial'},
    {id:'presencial',slug:'consulta-presencial',name:'Consulta Presencial',minutes:60,price:850,location:'Presencial'},
    {id:'adulto-mayor',slug:'consulta-para-adulto-mayor',name:'Consulta para Adulto Mayor',minutes:60,price:1000,location:'Presencial'},
    {id:'familiar',slug:'consulta-familiar',name:'Consulta Familiar',minutes:60,price:2360,location:'Presencial'}
  ];
  ...
  document.querySelector('#booking-form')?.addEventListener('submit',async event=>{event.preventDefault();if(!state.service||!state.date||!state.time)return message('Selecciona servicio, fecha y horario.',true);const fields=Object.fromEntries(new FormData(event.currentTarget));message('Guardando tu solicitud…');try{const response=await fetch('/api/bookings',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({...fields,serviceId:state.service.id,date:state.date,time:state.time,paymentOption:'pending'})});const data=await response.json();if(!response.ok)throw new Error(data.error||'No fue posible guardar la solicitud');document.querySelector('#booking-summary').innerHTML=`<strong>${state.service.name}</strong><br>${state.date} · ${state.time}<br>${state.service.location} · $${state.service.price} MXN`;document.querySelector('#booking-success').textContent=`Folio ${data.folio}. Recibimos tu solicitud. Makanuy te contactará para confirmar el horario y explicar el pago.`;show('complete');window.dataLayer?.push({event:'booking_request',service_id:state.service.id,booking_folio:data.folio})}catch(error){message(error.message,true)}});
